...
const handleRegister = async () => {
  if (validateFirmForm()) {
    const return_type: RETURN_TYPES = await register_firm(
      company_name.value,
...
const validateFirmForm = (): Boolean => {
  ...
  if (validateName(company_name.value) === false) {
    displayError(RETURN_TYPES.INVALID_FIRM_NAME_FORMAT);
    return false;
  }

  if (validateName(representative_name.value) === false) {
  ...
  if (validateCUI(cui.value) === false) {
